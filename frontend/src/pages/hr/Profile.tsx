...
const HRProfile: React.FC = () => {
  ...
  const handleApplyLeave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await hrService.requestLeave(leaveData);
      if (response.success) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 font-sans pb-12">
          ...
          <div className="flex-1 space-y-4">
            <div>
              <div className="flex flex-col md:flex-row md:items-center gap-2 mb-2">
                ...
                <Badge variant="brand" className="w-fit mx-auto md:mx-0 uppercase tracking-widest text-[10px] py-1 px-3">HR Manager</Badge>
              </div>
              <p className="text-gray-500 font-medium flex items-center justify-center md:justify-start gap-2">
                <Briefcase size={16} className="text-brand-500" /> {profile?.department} &bull; {profile?.branch?.name}
              </p>
            </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-brand-50 p-8 rounded-[40px] border border-brand-100 flex items-center justify-between">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h4 className="text-xl font-medium text-brand-700 leading-none  ">Node Administrator</h4>
                    <p className="text-brand-600 text-sm mt-2 font-medium">Verified HR Administrator for {profile?.branch?.name}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <Modal
        ...
        description="Submit a formal leave request for HR administrative records."
        maxWidth="xl"
      >
