  ...
  if (loading) {
    return (
      <div className="h-[calc(100vh-140px)] w-full flex flex-col items-center justify-center bg-white rounded-[40px] shadow-soft border border-slate-200">
        ...
        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest italic">Entering Student Secure Network...</p>
      </div>
    );
  }
  ...
  return (
    <>
        ...
        <div className="w-80 border-r border-slate-100 flex flex-col">
           <div className="p-6 border-b border-slate-50">
              ...
              <div className="flex items-center bg-slate-50 rounded-2xl px-4 py-2.5 border border-slate-100">
                 ...
                 <input type="text" placeholder="Search my tutors..." className="bg-transparent border-none focus:ring-0 text-xs font-bold ml-2 w-full uppercase tracking-wider placeholder:text-slate-300" />
              </div>
           </div>
              ...
              {contacts.length > 0 ? contacts.map((contact, idx) => (
                 ...
                 >
                    <div className="flex justify-between items-start mb-2">
                       ...
                       <span className={`text-[8px] font-bold uppercase tracking-widest ${selectedContact?.id === contact.id ? 'text-brand-200' : 'text-slate-400'}`}>Online</span>
                    </div>
                ...
                <div className="p-8 text-center opacity-40">
                   ...
                   <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed">No assigned tutors found for your class.</p>
                </div>
              )}
           </div>
        </div>
        ...
        <div className="flex-1 flex flex-col bg-slate-50/20">
           {selectedContact ? (
               ...
               <div ref={scrollRef} className="flex-1 p-6 sm:p-10 space-y-8 overflow-y-auto font-sans bg-slate-50/30">
                  {msgLoading && messages.length === 0 ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center opacity-40">
                       ...
                       <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Connect with your tutor for support.</p>
                    </div>
                  )}
               </div>
               ...
               <div className="p-8 bg-white border-t border-slate-100">
                  <form onSubmit={handleSend} className="flex items-center gap-4 bg-slate-50 rounded-[32px] p-2.5 border border-slate-100 focus-within:shadow-lg focus-within:border-brand-200 transition-all">
                     <input 
                       ...
                       placeholder="Type your secure curriculum query..." 
             ...
             <div className="flex-1 flex flex-col items-center justify-center p-20 text-center">
                ...
                <h3 className="text-2xl font-bold text-gray-900 tracking-tight">Select a Mentor</h3>
                <p className="text-slate-500 mt-2 max-w-sm font-medium">Choose an authorized tutor from the sidebar to start a secure academic conversation.</p>
             </div>
           )}
        </div>
      </div>
      ...
      <Modal
         ...
         description="Communicate directly with your assigned subject tutors in a secure environment."
         maxWidth="xl"
      >
         <form className="space-y-8" onSubmit={(e) => { e.preventDefault(); setIsModalOpen(false); }}>
            <div className="space-y-6">
               <div className="p-6 bg-brand-50 border border-brand-100 rounded-[32px] flex items-center gap-4">
                  ...
                  <p className="text-xs font-bold text-brand-600 uppercase tracking-widest leading-relaxed">Your tutor list is automatically generated based on your active class schedule.</p>
               </div>
            </div>

            <div className="flex justify-end gap-3 pt-4">
               <Button variant="outline" type="button" onClick={() => setIsModalOpen(false)} className="rounded-xl h-12 px-6">Close</Button>
               <Button type="button" onClick={() => setIsModalOpen(false)} className="rounded-xl h-12 px-8 shadow-premium bg-brand-500 text-white hover:bg-brand-600">Start Conversation</Button>
            </div>
         </form>
      </Modal>
    </>
