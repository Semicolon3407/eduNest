...
const submissions = [
  { id: 1, student: 'Ava Thompson', roll: 'D2401', file: 'calculus_set4_ava.pdf', date: 'Just now', size: '2.4 MB' },
  { id: 2, student: 'Leo Garcia', roll: 'D2402', file: 'leo_calc_submission.zip', date: '2 hours ago', size: '15.8 MB' },
  { id: 3, student: 'Mia Robinson', roll: 'D2403', file: 'set4_math.pdf', date: '5 hours ago', size: '1.1 MB' },
];

const TutorAssignments: React.FC = () => {
  const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
  const [_viewSubmissions, setViewSubmissions] = useState<number | null>(null);
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
                 ...
                 {activeAssignments.map((task) => (
                    <div key={task.id} className="p-6 hover:bg-slate-50/50 transition-all group">
                       ...
                       <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
                          <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                             ...
                             <div className="flex items-center gap-2 text-slate-700 font-bold text-xs">
                                <Calendar size={14} className="text-brand-500" /> {task.dueDate}
                             </div>
                          </div>
                          <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                          ...
                          <div className="sm:col-span-2 bg-slate-50 p-3 rounded-2xl border border-slate-100">
                             <div className="flex justify-between items-end mb-2">
                                ...
                                <p className="text-xs font-bold text-brand-600">{task.submissions}/{task.total} Students</p>
                             </div>
                             <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden">
                                <div className="h-full bg-brand-500 rounded-full" style={{ width: `${(task.submissions/task.total)*100}%` }}></div>
                             </div>
                          </div>
                       </div>
                       ...
                       <div className="mt-6 pt-4 border-t border-dashed border-slate-100 flex justify-end">
                          <button 
                            onClick={() => setViewSubmissions(task.id)}
              ...
              <div className="space-y-6">
                 {submissions.map((sub) => (
                    <div key={sub.id} className="relative pl-6 border-l-2 border-slate-100">
                       ...
                       <p className="text-[10px] font-bold text-brand-600 uppercase tracking-widest">{sub.date}</p>
                       <h5 className="text-sm font-bold text-slate-800 mt-1">{sub.student}</h5>
                       ...
                       <button className="flex items-center gap-2 text-[10px] font-black text-brand-500 uppercase tracking-widest mt-3 hover:text-brand-600">
                          <Download size={12} /> Download Submission ({sub.size})
                       </button>
                    </div>
                 ))}
              </div>
      ...
      >
        <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setIsCreateModalOpen(false); }}>
           <Input label="Assignment Title" placeholder="e.g. Calculus Problem Set 5" icon={FileText} required />
           ...
           <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                 ...
                 <div className="relative group">
                    ...
                    <select className="w-full h-12 bg-slate-50 border border-slate-200 rounded-xl px-12 font-bold text-sm appearance-none outline-none focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 transition-all">
              ...
              <Input label="Due Date" type="date" icon={Calendar} required />
           </div>
