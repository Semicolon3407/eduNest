...
import { 
  Mail, Phone, Briefcase, Shield, Calendar, 
  Loader2, Hash, User, CheckCircle2, XCircle, Clock, Plus
} from 'lucide-react';
...
const TutorProfile: React.FC = () => {
  ...
  const handleApplyLeave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await tutorService.applyLeave(leaveData);
      if (response.success) {
        toast.success('Leave application submitted successfully');
        setIsLeaveModalOpen(false);
        fetchProfile(); // Refresh to show new leave
      }
    } catch (error: any) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 font-sans pb-12">
          ...
          <div className="flex-1 space-y-4">
            <div>
              <div className="flex flex-col md:flex-row md:items-center gap-2 mb-2">
                <h1 className="text-3xl font-display font-medium text-gray-900 leading-none  ">{profileData?.firstName} {profileData?.lastName}</h1>
                <Badge variant="brand" className="w-fit mx-auto md:mx-0 uppercase tracking-widest text-[10px] py-1 px-3">Tutor</Badge>
              </div>
              <p className="text-gray-500 font-medium flex items-center justify-center md:justify-start gap-2">
                <Briefcase size={16} className="text-brand-500" /> {profileData?.department} &bull; {profileData?.branch?.name}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 pt-4 border-t border-slate-100">
              <div>
                ...
                <p className="text-sm font-bold text-gray-900 flex items-center gap-2">
                  <Hash size={16} className="text-brand-500" /> {profileData?.employeeId}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-sm font-bold text-gray-900 flex items-center gap-2 font-display">
                  {profileData?.attendanceStats?.rate || 100}%
                </p>
              </div>
              <div>
                ...
                <p className="text-sm font-bold text-brand-600 flex items-center gap-2">
                  <Calendar size={16} /> {new Date(profileData?.createdAt).toLocaleDateString()}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="animate-in slide-in-from-bottom-4 duration-500 font-sans">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white p-8 rounded-[40px] shadow-soft border border-slate-200">
                <h3 className="text-xl font-medium text-gray-900 mb-8 border-b border-slate-100 pb-4  ">Tutor Identity</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-10 gap-x-12">
                   <InfoItem icon={User} label="Full Name" value={`${profileData?.firstName} ${profileData?.lastName}`} />
              ...
              <div className="bg-brand-50 p-8 rounded-[40px] border border-brand-100 flex items-center justify-between">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h4 className="text-xl font-medium text-brand-700 leading-none  ">Academic Node</h4>
                    <p className="text-brand-600 text-sm mt-2 font-medium">Verified Tutor for {profileData?.branch?.name}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-900 p-8 rounded-[40px] text-white shadow-premium relative overflow-hidden group">
                ...
                <h3 className="text-lg font-medium mb-2 relative z-10  ">Faculty ID</h3>
                <p className="text-slate-400 text-xs mb-8 relative z-10">Institutional identifier for classroom access and node verification.</p>
          ...
          <div className="bg-white p-8 rounded-[40px] shadow-soft border border-slate-200">
            ...
            <div className="space-y-4">
              {profileData?.attendanceRecords?.length > 0 ? (
                profileData.attendanceRecords.map((record: any) => (
        ...
        {activeTab === 'leaves' && (
          <div className="bg-white p-8 rounded-[40px] shadow-soft border border-slate-200">
            <h3 className="text-xl font-medium text-gray-900 mb-8 border-b border-slate-100 pb-4">Leave Applications</h3>
            <div className="space-y-4">
              {profileData?.leaves?.length > 0 ? (
                profileData.leaves.map((leave: any) => (
      ...
      <Modal
        ...
        description="Submit a formal leave request for academic staff approval."
        maxWidth="xl"
      >
